  ...
  template: `
    <div class="p-8 max-w-4xl mx-auto h-full flex flex-col">
      ...
      <div *ngIf="result; else noResult" class="ks-card mt-8">
        <div class="border-b border-border-subtle pb-6 mb-6">
          ...
          <p class="text-body font-sans">
            Sinh viên thực hiện: <span class="font-medium text-primary">{{ result.studentName }}</span> ({{ result.studentId }})
          </p>
        </div>
        ...
        <div class="flex items-center justify-between p-6 bg-primary/5 border border-primary/20 rounded-sm mb-8">
          <div>
            ...
            <p class="font-display text-5xl font-bold text-primary">{{ result.finalScore }} / 10</p>
          </div>
        ...
        <div>
          <h3 class="font-sans text-lg font-medium text-heading mb-2">Nhận xét chung:</h3>
          <p class="text-body bg-surface-raised p-4 rounded-sm border border-border-subtle italic">
            "{{ result.comments }}"
          </p>
        </div>
      </div>
      ...
      <ng-template #noResult>
        <div class="mt-12 text-center text-muted p-12 border border-dashed border-border-subtle rounded-sm">
