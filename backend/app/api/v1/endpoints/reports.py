...
@router.post(
    ...
    description="Cho phép Sinh viên upload file báo cáo (PDF, DOCX, ZIP...). Tự động quản lý số phiên bản và kiểm tra dung lượng file (tối đa 20MB)."
)
async def upload_report_endpoint(
    topic_id: UUID = Form(..., description="ID của đề tài cần nộp báo cáo"),
    file: UploadFile = File(..., description="File báo cáo cần upload (tối đa 20MB)"),
...
async def get_topic_reports_endpoint(
    ...
    response_data = [ReportResponse.model_validate(r) for r in reports]
