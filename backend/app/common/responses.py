...
# Khai báo TypeVar để hỗ trợ kiểu dữ liệu linh hoạt (Generic) cho trường data
T = TypeVar('T')

class SuccessResponse(BaseModel, Generic[T]):
    """
    Schema chuẩn cho các phản hồi thành công (HTTP 200, 201,...).
    Tất cả API thành công ĐỀU PHẢI trả về đúng định dạng này.
    """
...
class ErrorDetail(BaseModel):
    ...
    """
    code: str                          # Mã lỗi định danh (VD: "INVALID_CREDENTIALS", "NOT_FOUND")
    details: Optional[Any] = None      # Thông tin chi tiết bổ sung (ví dụ danh sách các trường nộp thiếu)
...
class ErrorResponse(BaseModel):
    ...
    message: str                        # Thông báo lỗi hiển thị cho người dùng
