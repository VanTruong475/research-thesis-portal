...
class ReportService:
    ...
    async def upload_report(
        ...
        if not topic:
            raise ResourceNotFoundException("Đề tài không tồn tại.")
        ...
        if file_size > MAX_FILE_SIZE_BYTES:
            raise BusinessRuleException("Kích thước file vượt quá giới hạn tối đa cho phép (20MB).")

        if file_size == 0:
            raise BusinessRuleException("File nộp vào không được rỗng.")

        # 3. Tính toán số phiên bản tiếp theo cho đề tài này (FR-17)
        stmt_max_version = (
            select(func.coalesce(func.max(Report.version), 0))
            .where(Report.topic_id == topic_id)
        )
