...
class CreateProgressLogRequest(BaseModel):
    ...
    milestone_id: Optional[UUID] = Field(default=None, description="ID của mốc tiến độ (nếu có)")
    # Nội dung chi tiết các công việc đã làm
    content: str = Field(..., min_length=5, description="Nội dung báo cáo tiến độ (tối thiểu 5 ký tự)")
...
class AddTeacherCommentRequest(BaseModel):
    ...
    teacher_comment: str = Field(..., min_length=2, description="Nội dung nhận xét của giảng viên")
...
class ProgressLogResponse(BaseModel):
    ...
    class Config:
        # Cho phép Pydantic đọc trực tiếp thuộc tính từ SQLAlchemy Model
