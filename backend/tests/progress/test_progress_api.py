...
async def create_progress_log(
    ...
    *,
    registration_id,
    student_id,
    content: str = "Completed the first implementation milestone.",
) -> ProgressLog:
    log = ProgressLog(
...
async def create_progress_context(
    test_session: AsyncSession,
    *,
...
@pytest.mark.asyncio
async def test_student_can_submit_progress_for_own_approved_registration_in_progress_period(
    client: AsyncClient,
    test_session: AsyncSession,
):
    _, _, _, student, _, _, registration = await create_progress_context(test_session)
    ...
    response = await client.post(
        ...
        json={
            ...
            "content": "Finished the first research tasks.",
        },
    )
    ...
    assert data["content"] == "Finished the first research tasks."


@pytest.mark.asyncio
async def test_student_cannot_submit_progress_for_another_students_registration(
    client: AsyncClient,
    test_session: AsyncSession,
):
    _, _, _, _student, _, _, registration = await create_progress_context(test_session)
    ...
    response = await client.post(
        "/api/v1/progress",
        headers=headers,
        json={"registration_id": str(registration.id), "content": "Trying to submit."},
    )
...
async def test_student_cannot_submit_progress_when_registration_is_not_approved(
    client: AsyncClient,
    test_session: AsyncSession,
    registration_status: RegistrationStatus,
):
    _, _, _, student, _, _, registration = await create_progress_context(
        test_session,
    ...
    assert response.json()["error"]["code"] == "PROGRESS_REGISTRATION_NOT_APPROVED"
...
async def test_student_cannot_submit_progress_outside_in_progress_academic_period(
    client: AsyncClient,
    test_session: AsyncSession,
    ...
    assert response.status_code == 400
    assert response.json()["error"]["code"] == "PROGRESS_PERIOD_NOT_IN_PROGRESS"


@pytest.mark.asyncio
async def test_official_supervisor_can_comment_on_progress_log(
    client: AsyncClient,
    test_session: AsyncSession,
):
    _, _, supervisor, student, _, _, registration = await create_progress_context(test_session)
    log = await create_progress_log(
        test_session,
    ...
    response = await client.post(
        f"/api/v1/progress/{log.id}/comments",
        headers=headers,
        json={"teacher_comment": "Good progress."},
    )

    assert response.status_code == 200
    data = response.json()["data"]
    assert data["teacher_comment"] == "Good progress."
    assert data["commented_at"] is not None


@pytest.mark.asyncio
async def test_non_supervisor_lecturer_cannot_comment_on_progress_log(
    client: AsyncClient,
    test_session: AsyncSession,
):
    _, _, _supervisor, student, _, _, registration = await create_progress_context(test_session)
    ...
    response = await client.post(
        f"/api/v1/progress/{log.id}/comments",
        headers=headers,
        json={"teacher_comment": "Unauthorized comment."},
    )
...
@pytest.mark.asyncio
async def test_topic_proposer_who_is_not_assigned_supervisor_cannot_comment(
    client: AsyncClient,
    test_session: AsyncSession,
):
    ...
    response = await client.post(
        f"/api/v1/progress/{log.id}/comments",
        headers=headers,
        json={"teacher_comment": "I proposed this topic."},
    )
...
@pytest.mark.asyncio
async def test_anonymous_user_cannot_read_progress_logs(
    client: AsyncClient,
    test_session: AsyncSession,
):
    _, _, _, _student, _, _, registration = await create_progress_context(test_session)

    response = await client.get(f"/api/v1/registrations/{registration.id}/progress")
...
@pytest.mark.asyncio
async def test_student_can_read_only_own_registration_progress_logs(
    client: AsyncClient,
    test_session: AsyncSession,
):
    _, _, _, student, _, _, registration = await create_progress_context(test_session)
    ...
    response = await client.get(
        f"/api/v1/registrations/{registration.id}/progress",
        headers=student_headers,
    )
    assert response.status_code == 200
    assert response.json()["data"][0]["id"] == str(log.id)
...
@pytest.mark.asyncio
async def test_lecturer_can_read_only_supervised_registration_progress_logs(
    client: AsyncClient,
    test_session: AsyncSession,
):
    _, _, supervisor, student, _, _, registration = await create_progress_context(test_session)
...
@pytest.mark.asyncio
async def test_admin_can_read_progress_logs_for_oversight(
    client: AsyncClient,
    test_session: AsyncSession,
):
    admin, _, _, student, _, _, registration = await create_progress_context(test_session)
    log = await create_progress_log(
        test_session,
        registration_id=registration.id,
        student_id=student.id,
...
@pytest.mark.asyncio
async def test_admin_cannot_comment_as_supervisor(
    client: AsyncClient,
    test_session: AsyncSession,
):
    admin, _, _supervisor, student, _, _, registration = await create_progress_context(test_session)
    log = await create_progress_log(
        test_session,
        registration_id=registration.id,
        student_id=student.id,
    ...
    response = await client.post(
        f"/api/v1/progress/{log.id}/comments",
        headers=headers,
        json={"teacher_comment": "Admin comment."},
    )
